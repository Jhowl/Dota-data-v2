...
export async function generateMetadata({ params }: LeaguePageProps) {
  ...
  const description = matchCount
    ? `${league.name} Dota 2 statistics: ${formatNumber(matchCount)} matches across ${formatNumber(teamCount)} teams, ${avgKills.toFixed(1)} avg kills per match. Pick & ban analysis, top performers, team participation and downloadable data.`
    : `Statistics, match results, and team analysis for ${league.name}. Pick & ban data, top performers, and team standings on DotaData.`;

  const title = `${league.name} - Dota 2 Statistics, Standings & Match Data`;
