...
export async function generateMetadata({ params }: PickBanPageProps) {
  ...
  const description = drafts
    ? `Pick & ban analysis for ${league.name}: ${formatNumber(drafts)} drafts analysed, ${formatNumber(heroes)} unique heroes seen, hero winrates, draft priority, side splits and per-team breakdowns.`
    : `Pick & ban analysis for ${league.name}. Hero contest rates, winrates, draft order priority and per-team breakdowns on DotaData.`;

  const title = `${league.name} - Pick & Ban Analysis | Hero Draft Stats`;
  ...
  return {
    ...
    keywords: [
      ...
      "Dota 2 hero winrate",
      "Dota 2 contest rate",
      "Dota 2 priority pick",
      "Dota 2 priority ban",
      `${league.name} pick ban`,
      `${league.name} draft`,
      `${league.name} meta`,
    ],
    openGraph: {
      ...
      description,
      type: "article",
      url: `/leagues/${league.slug}/pick-ban`,
      siteName: "DotaData",
    },
    twitter: {
    ...
    alternates: {
      canonical: `/leagues/${league.slug}/pick-ban`,
    },
  };
}
